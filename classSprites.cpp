...
void sprites::exportMapping(void){
	if(load_file_generic("Save mapping to:",true)){
		FILE*fp=fopen(the_file.c_str(),"w");
		fputs(";sprite mapping generated by ",fp);
		fputs(rtVersionStr,fp);
		fputc('\n',fp);
		fputs(name.c_str(),fp);
		fputs(":\n",fp);
		...
		for(uint32_t j=0;j<amt;++j){
			...
			for(uint32_t i=0;i<groups[j].list.size();++i)
				fprintf(fp,"\tdc.b %d,%u,%u,%u,%d\n",groups[j].offy[i],((groups[j].list[i].w-1)<<2)|(groups[j].list[i].h-1),(groups[j].list[i].prio<<7)|(groups[j].list[i].palrow<<5)|(groups[j].list[i].vflip<<4)|(groups[j].list[i].hflip<<3)|((groups[j].list[i].starttile>>8)&7),groups[j].list[i].starttile&255,groups[j].offx[i]);
		}
		fclose(fp);
	}
}
