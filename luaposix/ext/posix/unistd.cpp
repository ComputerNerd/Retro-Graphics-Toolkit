...
runexec(lua_State *L, int use_shell)
{
	...
	n = lua_objlen(L, 2);
	argv = (char**)lua_newuserdata(L, (n + 2) * sizeof(char*));
...
Pgetcwd(lua_State *L)
{
	...
	if ((b = (char*)lalloc(ud, NULL, 0, (size_t)size + 1)) == NULL)
...
Pgetgroups(lua_State *L)
{
	...
	else
	{
		...
		group = (gid_t*)lua_newuserdata(L, sizeof(*group) * n_group_slots);
