...
static int lua_chunk_getTileRow(lua_State*L) {
	...
	lua_pushinteger(L, projects->at(projectIDX).tms->maps[entryIDX].get_tileRow(luaL_optinteger(L, 2, 0), luaL_optinteger(L, 3, 0), luaL_optinteger(L, 4, 0)));
	return 1;
}
...
static int chunkEntry__get_(lua_State *L) {
	...
	if (type == LUA_TSTRING) {
		...
		const size_t columnIDX = idxPtr[2];
		const size_t entryIDX = idxPtr[3];
		const class ChunkClass *chunk = projects->at(projectIDX).Chunk;
