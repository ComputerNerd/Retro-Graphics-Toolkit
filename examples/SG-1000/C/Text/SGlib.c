...
const unsigned char VDPReg_init[8]={
                  0x00, // Text mode.
                  0xd0,	// 16KB, screen off, VBlank IRQ, sprite 8x8, no zoom
                  0x06,	// PN bits 13-10 = 0 1 1 0	   (address = $1800)
                  0xff,	// CT bits 13-7  = 1 x x x x x x x (address = $2000)
                  0x00,	// PG bits 13-11 = 0 x x	   (address = $0000)
                  0x20,	// SA bits 13-7  = 0 1 1 0 1 1 0   (address = $1B00)
                  0x00,	// SG bits 13-11 = 1 1 1	   (address = $3800)
                  0xF1	// text color (unused in Mode2) / backdrop (black)
};

/* the VDP registers #0 and #1 'shadow' RAM */
unsigned char	VDPReg[2]={0x00, 0xd0};
